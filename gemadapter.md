...
- **RUN TRACEABILITY:** require a compact log of exact searches and the candidate source IDs they surfaced. For each video-derived item, request a timestamp or closest available locator, inspected modality, and transcript language / caption provenance when determinable. When a retrieved transcript exposes timestamped lines or segments, the exact start timestamp attached to the relevant line/segment is a sufficient locator; do not require an end timestamp or interval, and do not estimate or interpolate one. If no timestamp is exposed, state that and give the closest available locator. Ask the Gem to identify the underlying work separately from the uploader or recording.
