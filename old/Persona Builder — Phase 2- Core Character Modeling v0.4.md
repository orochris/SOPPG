...
The Core captures the smallest useful set of recurring behavioral patterns needed to recognize the character across contexts.
...
`MODEL SCOPE` must establish that the model is:
...
6. Preserve material counterevidence.
7. Keep hypotheses explicitly hypothetical.
8. Follow the canonical section order.
9. Use canonical field names when relevant.
