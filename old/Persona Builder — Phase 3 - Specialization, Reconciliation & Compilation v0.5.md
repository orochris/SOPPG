...
### v0.5 — Source-Informed Frozen Core → Specialized Persona Prompt
...
Transform a **frozen Phase 2 Core Character Model** into a target-context persona by applying explicit specialization lenses, reconciling their interactions, and compiling the accepted result into a **standalone deployment-ready persona prompt**.
...
- use upstream evidence to silently replace the frozen Core;
...
If a specialization reveals a material problem in the Core, record it explicitly as `REVISE` or `VERSION-SPLIT`.

Core revision belongs to a subsequent Phase 2 execution.

---

## FROZEN CORE RULE

The Phase 2 Core is authoritative.
...
- preserve it;
- qualify its expression;
- adapt its expression;
- add target-specific context;
- identify a possible Core problem;
- mark a version split;
- reject an incompatible specialization;
- impose technical realization constraints.

Phase 3 may **not** silently replace the Core.

A substantive disagreement with the frozen Core must be represented explicitly as a typed delta or unresolved issue.

---

## SINGLE-EXECUTION RULE

Phase 3 is a **single specialization and compilation pass**.

The specialization lenses are not serial character rewrites.

They are analytical views over the same frozen Core.
...
# 3. FROZEN CORE REFERENCE

This is a **minimal reference layer**, not a rewritten Core.
...
- RELEVANT MODULATION / FUNCTIONAL BEHAVIOR:

Only include Core material necessary to interpret the specialization.

Do not restate the entire Core unless required for traceability.

Do not modify Core meaning for convenience.

---

## 3.1 CORE INVARIANT RULE

Identify the aspects of the Core that specialization must preserve.
...
## 3.2 CORE UNCERTAINTY RULE

Carry forward material Phase 2 uncertainties.
...
Its purpose is to recover source-level detail that may have been lost when Phase 1 was compressed into the evidence ledger and Phase 2 into the frozen Core.

### REQUIRED PROCESS

1. Start from the frozen Core and target conditions.
...
Do not reopen questions already settled by the frozen Core merely because a source contains an interesting alternative interpretation.

If raw source material appears to materially contradict the frozen Core, record a `REVISE` or `VERSION-SPLIT` issue rather than silently changing the model.
...
The Relationship lens determines how the target relationship changes expression of the frozen Core.
...
## 5.5 DIALOGUE & BEHAVIORAL REALIZATION LENS

The Dialogue & Behavioral Realization lens determines **how the specialized character should actually sound and behave in live interaction**, without creating a new Core.

It exists because a semantically correct Core may still be under-specified for an interactive language model. Concrete dialogue patterns, examples, and selective quirks may therefore be needed to make the intended realization reproducible.
...
For conversational LLM roleplay, when the Dialogue & Behavioral Realization lens is materially applicable, the compiler should ordinarily include all three of:

- dialogue patterns;
- example voice lines;
- behavioral-quirk examples.

They need not be numerous. Prefer a small, diverse set that covers distinct interactional situations.

Omit one only when it would add no useful information, conflict with the target, or exceed a justified deployment budget. Do not satisfy this guidance through redundant examples.
...
### 1. Frozen Core integrity

A specialization cannot silently overwrite the Core.
...
> **FROZEN CORE + ACCEPTED SPECIALIZATION DELTAS + REALIZATION MATERIAL + RECONCILIATION DECISIONS + TECHNICAL DEPLOYMENT CONSTRAINTS**
...
The compiler must preserve the semantic Core while realizing accepted specialization.
...
- replace the frozen Core with a newly interpreted character.
...
## 8.1 CORE PRESERVATION CHECK

Ask:

> Does the compiled persona prompt preserve the accepted semantic Core?
...
## 8.4 LANGUAGE CONTAMINATION CHECK
...
## 8.5 DEPLOYMENT CONTAMINATION CHECK
...
## 8.6 RECOMPILATION STABILITY CHECK
...
The same underlying Core should remain recoverable.
...
## 8.8 FINAL PERSONA PROMPT CHECK
...
For interactive RP, a prompt that contains only abstract trait descriptions may be semantically correct yet under-demonstrated; where useful, verify that dialogue patterns and examples make the intended behavior recoverable.
...
- the frozen Core remains intact;
...
The Phase 2 Core remains immutable.

Phase 3 must never silently return a rewritten Core.

When a specialization exposes a possible defect in the Core, record:
...
and preserve the existing frozen Core.

A future Core revision belongs to a new Phase 2 execution.
...
```text
CORE
What remains the same character.
...
1. Verify that the Phase 2 Core is frozen.
2. Read the Core before applying specialization.
...
> **frozen Core + explicit specialization deltas + reconciled realization + standalone persona prompt + audit**
